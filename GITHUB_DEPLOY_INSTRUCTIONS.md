...
- В `vite.config.js` уже выставлен `base: './'`, что подходит для статического хостинга с вложенным путем.
...
- **404 / белая страница**: проверьте, что не переопределили `base` в `vite.config.js`.
